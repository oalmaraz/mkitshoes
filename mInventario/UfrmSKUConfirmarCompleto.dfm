object frmSKUConfirmarCompleto: TfrmSKUConfirmarCompleto
  Left = 638
  Top = 202
  Caption = 'SKU Confirmacion Completa'
  ClientHeight = 237
  ClientWidth = 499
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  PixelsPerInch = 96
  TextHeight = 13
  object paTitulo: TPanel
    Left = 0
    Top = 0
    Width = 499
    Height = 20
    Align = alTop
    BevelOuter = bvNone
    Caption = 'SKU Confirmacion Completa'
    Color = 12615680
    Font.Charset = ANSI_CHARSET
    Font.Color = clWhite
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    ExplicitWidth = 507
  end
  object paBotones: TPanel
    Left = 0
    Top = 197
    Width = 499
    Height = 40
    Align = alBottom
    TabOrder = 4
    ExplicitTop = 202
    ExplicitWidth = 507
    object buSalir: TBitBtn
      Left = 420
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Regresar'
      Glyph.Data = {
        
        A7B7A7A7B7A7A7B7A6A6B6A6A6B8DBDBE1FFFFFFFFFFFFFFFFFF}
      ModalResult = 5
      TabOrder = 0
    end
  end
  object paMaterial: TPanel
    Left = 0
    Top = 20
    Width = 499
    Height = 48
    Align = alTop
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 1
    ExplicitWidth = 507
    object Label1: TLabel
      Left = 8
      Top = 7
      Width = 46
      Height = 16
      Caption = '&Material'
      FocusControl = edMaterial
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 196
      Top = 7
      Width = 65
      Height = 16
      Caption = 'Descripcion'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object edCBarras: TEdit
      Left = 60
      Top = 5
      Width = 133
      Height = 21
      TabStop = False
      Color = clBackground
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Text = '9999999999999'
    end
    object edDescripcion: TEdit
      Left = 196
      Top = 25
      Width = 296
      Height = 21
      TabStop = False
      Color = clBackground
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
      Text = 'ningun material'
    end
    object edMaterial: TEdit
      Left = 8
      Top = 25
      Width = 185
      Height = 21
      CharCase = ecUpperCase
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnKeyPress = edMaterialKeyPress
    end
  end
  object paMedidas: TPanel
    Left = 0
    Top = 68
    Width = 499
    Height = 51
    Align = alTop
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 2
    ExplicitWidth = 507
    object laMedida: TLabel
      Left = 8
      Top = 7
      Width = 41
      Height = 16
      Caption = 'M&edida'
      FocusControl = cbMedidas
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object sbRefreshMedidas: TSpeedButton
      Left = 90
      Top = 4
      Width = 21
      Height = 19
      Hint = 'Actualizar'
      Flat = True
      Glyph.Data = {
        
        04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
      ParentShowHint = False
      ShowHint = True
      OnClick = sbRefreshMedidasClick
    end
    object laColor: TLabel
      Left = 114
      Top = 7
      Width = 30
      Height = 16
      Caption = 'C&olor'
      FocusControl = cbColores
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object sbRefreshColores: TSpeedButton
      Left = 172
      Top = 4
      Width = 21
      Height = 19
      Hint = 'Actualizar'
      Flat = True
      Glyph.Data = {
        
        04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
      ParentShowHint = False
      ShowHint = True
      OnClick = sbRefreshColoresClick
    end
    object cbMedidas: TDBLookupComboBox
      Left = 8
      Top = 25
      Width = 103
      Height = 21
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object cbColores: TDBLookupComboBox
      Left = 114
      Top = 25
      Width = 79
      Height = 21
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
  end
  object paCantidad: TPanel
    Left = 0
    Top = 119
    Width = 499
    Height = 66
    Align = alTop
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 3
    ExplicitWidth = 507
    object Label7: TLabel
      Left = 232
      Top = 12
      Width = 35
      Height = 19
      Caption = '&Serie'
      FocusControl = edSerie
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object edSerie: TEdit
      Left = 8
      Top = 35
      Width = 482
      Height = 27
      CharCase = ecUpperCase
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      MaxLength = 100
      ParentFont = False
      TabOrder = 0
      OnKeyPress = edSerieKeyPress
    end
  end
  object spSKU_CONFIRMACION_COMPLETA: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'SKU_CONFIRMACION_COMPLETA'
    Left = 440
    Top = 80
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_MI'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_MATERIAL'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_MEDIDA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_COLOR'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_SERIE'
        ParamType = ptInput
      end>
  end
  object qyMateriales_Medidas: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VER_MATERIALES_MEDIDAS(:P_MATERIAL)')
    Left = 312
    Top = 81
    ParamData = <
      item
        DataType = ftString
        Name = 'P_MATERIAL'
        ParamType = ptUnknown
      end>
    object qyMateriales_MedidasR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyMateriales_MedidasR_ID_MEDIDA: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID_MEDIDA'
      ReadOnly = True
    end
    object qyMateriales_MedidasR_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCRIPCION'
      ReadOnly = True
    end
    object qyMateriales_MedidasR_CODIGO_BARRAS: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CODIGO_BARRAS'
      ReadOnly = True
    end
  end
  object qyMateriales_Colores: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VER_MATERIALES_COLORES(:P_MATERIAL)')
    Left = 249
    Top = 81
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'P_MATERIAL'
        ParamType = ptUnknown
      end>
    object qyMateriales_ColoresR_ORDEN: TIntegerField
      FieldName = 'R_ORDEN'
      Origin = 'VER_MATERIALES_COLORES.R_ORDEN'
    end
    object qyMateriales_ColoresR_ID: TIntegerField
      FieldName = 'R_ID'
      Origin = 'VER_MATERIALES_COLORES.R_ID'
    end
    object qyMateriales_ColoresR_COLOR: TIntegerField
      FieldName = 'R_COLOR'
      Origin = 'VER_MATERIALES_COLORES.R_COLOR'
    end
    object qyMateriales_ColoresR_COLOR_NOMBRE: TIBStringField
      FieldName = 'R_COLOR_NOMBRE'
      Origin = 'VER_MATERIALES_COLORES.R_COLOR_NOMBRE'
      Size = 50
    end
  end
  object dsColores: TDataSource
    DataSet = qyMateriales_Colores
    Left = 280
    Top = 81
  end
  object dsMateriales_Medidas: TDataSource
    DataSet = qyMateriales_Medidas
    Left = 344
    Top = 81
  end
end
