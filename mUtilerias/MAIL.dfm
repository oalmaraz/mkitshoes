object frmMail: TfrmMail
  Left = 480
  Top = 136
  Caption = 'Mail'
  ClientHeight = 314
  ClientWidth = 377
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label7: TLabel
    Left = 6
    Top = 83
    Width = 93
    Height = 13
    Caption = 'Servidor de Correos'
    Transparent = True
  end
  object Label8: TLabel
    Left = 8
    Top = 211
    Width = 31
    Height = 13
    Caption = 'Puerto'
    Transparent = True
  end
  object Label9: TLabel
    Left = 8
    Top = 168
    Width = 27
    Height = 13
    Caption = 'Clave'
    Transparent = True
  end
  object Label1: TLabel
    Left = 6
    Top = 126
    Width = 91
    Height = 13
    Caption = 'Nombre de Usuario'
    Transparent = True
  end
  object paEncabezado: TPanel
    Left = 0
    Top = 0
    Width = 377
    Height = 68
    Align = alTop
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 0
    ExplicitWidth = 385
    object Bevel1: TBevel
      Left = 0
      Top = 64
      Width = 385
      Height = 4
      Align = alBottom
      Shape = bsBottomLine
    end
  end
  object edServidor: TEdit
    Left = 6
    Top = 99
    Width = 361
    Height = 21
    MaxLength = 100
    TabOrder = 1
  end
  object edPuerto: TEdit
    Left = 6
    Top = 227
    Width = 91
    Height = 21
    MaxLength = 10
    TabOrder = 4
  end
  object edClave: TEdit
    Left = 6
    Top = 184
    Width = 235
    Height = 21
    MaxLength = 50
    PasswordChar = #176
    TabOrder = 3
  end
  object paBotones: TPanel
    Left = 0
    Top = 274
    Width = 377
    Height = 40
    Align = alBottom
    TabOrder = 5
    ExplicitTop = 279
    ExplicitWidth = 385
    object buGuardar: TBitBtn
      Left = 216
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Guardar'
      Glyph.Data = {
        
        0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000}
      NumGlyphs = 2
      TabOrder = 0
      OnClick = buGuardarClick
    end
    object buSalir: TBitBtn
      Left = 294
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Salir'
      Glyph.Data = {
        
        A7B7A7A7B7A7A7B7A6A6B6A6A6B8DBDBE1FFFFFFFFFFFFFFFFFF}
      TabOrder = 1
      OnClick = buSalirClick
    end
  end
  object edUsuario: TEdit
    Left = 6
    Top = 142
    Width = 235
    Height = 21
    MaxLength = 50
    TabOrder = 2
  end
  object spCFG_MAIL: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'CFG_MAIL'
    Left = 168
    Top = 224
    ParamData = <
      item
        DataType = ftInteger
        Name = 'R_ID'
        ParamType = ptOutput
      end
      item
        DataType = ftString
        Name = 'P_SERVIDOR'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_USUARIO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_CLAVE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_PUERTO'
        ParamType = ptInput
      end>
  end
  object qyMail: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM MAIL')
    Left = 128
    Top = 224
    object qyMailID: TIntegerField
      FieldName = 'ID'
      Origin = 'MAIL.ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qyMailMAIL_SERVER: TIBStringField
      FieldName = 'MAIL_SERVER'
      Origin = 'MAIL.MAIL_SERVER'
      Size = 100
    end
    object qyMailUSER_NAME: TIBStringField
      FieldName = 'USER_NAME'
      Origin = 'MAIL.USER_NAME'
      Size = 50
    end
    object qyMailUSER_PASS: TIBStringField
      FieldName = 'USER_PASS'
      Origin = 'MAIL.USER_PASS'
      Size = 50
    end
    object qyMailPORT: TIntegerField
      FieldName = 'PORT'
      Origin = 'MAIL.PORT'
    end
  end
end
