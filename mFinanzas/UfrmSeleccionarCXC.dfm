object frmSeleccionarCXC: TfrmSeleccionarCXC
  Left = 252
  Top = 240
  Caption = 'Seleccionar Cuentas por Cobrar'
  ClientHeight = 0
  ClientWidth = 124
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object paBusqueda: TPanel
    Left = 0
    Top = 0
    Width = 124
    Height = 52
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Seleccion'
    Color = 12505816
    TabOrder = 0
    ExplicitWidth = 718
    object buBusqueda: TBitBtn
      Left = 4
      Top = 22
      Width = 188
      Height = 27
      Caption = 'CXC.REFERENCIA'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        FF7F1F7C1F7C}
      ParentFont = False
      Spacing = 9
      TabOrder = 3
      OnClick = buBusquedaClick
    end
    object buEjecutar: TBitBtn
      Left = 339
      Top = 20
      Width = 68
      Height = 31
      Caption = '&Ejecutar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
      ParentFont = False
      TabOrder = 1
      OnClick = buEjecutarClick
    end
    object buTodos: TBitBtn
      Left = 409
      Top = 20
      Width = 68
      Height = 31
      Caption = '&Todos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = buTodosClick
    end
    object edDato: TEdit
      Left = 195
      Top = 25
      Width = 141
      Height = 21
      CharCase = ecUpperCase
      TabOrder = 0
      OnKeyPress = edDatoKeyPress
    end
  end
  object sbHijo: TPanel
    Left = 0
    Top = -22
    Width = 124
    Height = 22
    Align = alBottom
    TabOrder = 1
    ExplicitTop = 424
    ExplicitWidth = 718
    object deID: TDBEdit
      Left = 1
      Top = 3
      Width = 88
      Height = 18
      BorderStyle = bsNone
      DataField = 'ID'
      DataSource = dsBusqueda
      Enabled = False
      TabOrder = 0
    end
    object buInicio: TBitBtn
      Left = 93
      Top = 3
      Width = 26
      Height = 18
      Glyph.Data = {
        }
      NumGlyphs = 2
      TabOrder = 1
      OnClick = buInicioClick
    end
    object buAnterior: TBitBtn
      Left = 123
      Top = 3
      Width = 26
      Height = 18
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      NumGlyphs = 2
      TabOrder = 2
      OnClick = buAnteriorClick
    end
    object buSiguiente: TBitBtn
      Left = 153
      Top = 3
      Width = 26
      Height = 18
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF0000}
      NumGlyphs = 2
      TabOrder = 5
      OnClick = buSiguienteClick
    end
    object buFin: TBitBtn
      Left = 183
      Top = 3
      Width = 26
      Height = 18
      Glyph.Data = {
        }
      NumGlyphs = 2
      TabOrder = 6
      OnClick = buFinClick
    end
    object ibSalir: TBitBtn
      Left = 283
      Top = 3
      Width = 36
      Height = 18
      Hint = 'Salir'
      Caption = '&Salir'
      Glyph.Data = {
        
        0000}
      Layout = blGlyphRight
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      OnClick = ibSalirClick
    end
    object sbUp: TBitBtn
      Left = 213
      Top = 3
      Width = 31
      Height = 18
      TabOrder = 3
    end
    object sbDown: TBitBtn
      Left = 248
      Top = 3
      Width = 31
      Height = 18
      TabOrder = 4
    end
  end
  object dgExplorar: TDBGrid
    Left = 0
    Top = 52
    Width = 124
    Height = 372
    Align = alClient
    Color = clInfoBk
    DataSource = dsBusqueda
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDblClick = dgExplorarDblClick
    OnKeyUp = dgExplorarKeyUp
  end
  object IBDatabase1: TIBDatabase
    DatabaseName = 'D:\proyectos\apm\Bases\SISTEMA.AWR'
    Params.Strings = (
      'user_name=SYSDBA'
      'password=masterkey')
    LoginPrompt = False
    DefaultTransaction = IBTransaction1
    ServerType = 'IBServer'
    SQLDialect = 1
    Left = 112
    Top = 128
  end
  object IBTransaction1: TIBTransaction
    Params.Strings = (
      'read_committed'
      'rec_version'
      'nowait')
    Left = 112
    Top = 160
  end
  object dsBusqueda: TDataSource
    DataSet = qyBusqueda
    Left = 208
    Top = 160
  end
  object qyBusqueda: TIBQuery
    Database = IBDatabase1
    Transaction = IBTransaction1
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      
        'SELECT CXC.ID, CXC.REFERENCIA, CXC.CLIENTE, CXC.EXTENSION, CXC.A' +
        'LMACEN, CXC.NUMERO_DE_DOCUMENTOS, CXC.DOCUMENTO, CXC.FECHA, CXC.' +
        'FECHA_VENCIMIENTO, CXC.IMPORTE_INICIAL, CXC.SALDO_ACTUAL + CXC.F' +
        'INANCIAMIENTO + CXC.GASTOS + CXC.MORATORIOS + CXC.OTROS, S.CLAVE' +
        ', S.NOMBRE, S.RAZON_SOCIAL, S.FACTURAR_A, A.NOMBRE, S.NOMBRE EXT' +
        'ENSION_NOMBRE'
      
        'FROM CUENTAS_POR_COBRAR CXC, SUJETOS_CLIENTES SC, SUJETOS S, ALM' +
        'ACENES A'
      
        'WHERE (CXC.CLIENTE = SC.ID) AND (SC.SUJETO = S.ID) AND (CXC.ALMA' +
        'CEN = A.ID) AND (CXC.ESTATUS = "Por Cobrar")')
    Left = 208
    Top = 128
    object qyBusquedaID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object qyBusquedaREFERENCIA: TIBStringField
      FieldName = 'REFERENCIA'
    end
    object qyBusquedaCLIENTE: TIntegerField
      FieldName = 'CLIENTE'
      Required = True
    end
    object qyBusquedaEXTENSION: TIntegerField
      FieldName = 'EXTENSION'
    end
    object qyBusquedaALMACEN: TIntegerField
      FieldName = 'ALMACEN'
      Required = True
    end
    object qyBusquedaNUMERO_DE_DOCUMENTOS: TIntegerField
      FieldName = 'NUMERO_DE_DOCUMENTOS'
    end
    object qyBusquedaDOCUMENTO: TIntegerField
      FieldName = 'DOCUMENTO'
    end
    object qyBusquedaFECHA: TDateTimeField
      FieldName = 'FECHA'
    end
    object qyBusquedaFECHA_VENCIMIENTO: TDateTimeField
      FieldName = 'FECHA_VENCIMIENTO'
    end
    object qyBusquedaIMPORTE_INICIAL: TFloatField
      FieldName = 'IMPORTE_INICIAL'
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyBusquedaF_1: TFloatField
      FieldName = 'F_1'
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyBusquedaCLAVE: TIBStringField
      FieldName = 'CLAVE'
      Size = 50
    end
    object qyBusquedaNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Size = 100
    end
    object qyBusquedaNOMBRE1: TIBStringField
      FieldName = 'NOMBRE1'
      Size = 100
    end
    object qyBusquedaRAZON_SOCIAL: TIBStringField
      FieldName = 'RAZON_SOCIAL'
      Size = 100
    end
    object qyBusquedaFACTURAR_A: TIBStringField
      FieldName = 'FACTURAR_A'
      Size = 12
    end
    object qyBusquedaEXTENSION_NOMBRE: TIBStringField
      FieldName = 'EXTENSION_NOMBRE'
      Size = 100
    end
  end
  object pmCampos: TPopupMenu
    Left = 256
    Top = 128
    object Id1: TMenuItem
      Caption = 'Id'
      OnClick = Id1Click
    end
    object Referencia1: TMenuItem
      Caption = 'Referencia'
      OnClick = Referencia1Click
    end
    object Cliente1: TMenuItem
      Caption = 'Cliente'
      OnClick = Cliente1Click
    end
    object Extension1: TMenuItem
      Caption = 'Extension'
      OnClick = Extension1Click
    end
  end
end
