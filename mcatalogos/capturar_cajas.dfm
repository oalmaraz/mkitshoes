inherited frmCapturar_cajas: TfrmCapturar_cajas
  Left = 481
  Top = 206
  Caption = 'Cajas'
  ClientHeight = 468
  ClientWidth = 653
  ExplicitWidth = 669
  ExplicitHeight = 507
  PixelsPerInch = 96
  TextHeight = 13
  inherited paLienzo: TPanel
    Width = 653
    Height = 386
    ExplicitWidth = 653
    ExplicitHeight = 386
    object Label1: TLabel
      Left = 8
      Top = 8
      Width = 21
      Height = 13
      Caption = 'Ca&ja'
      FocusControl = deCaja
      Transparent = True
    end
    object Label2: TLabel
      Left = 269
      Top = 8
      Width = 41
      Height = 13
      Caption = '&Almacen'
      FocusControl = leAlmacenes
      Transparent = True
    end
    object Label3: TLabel
      Left = 419
      Top = 8
      Width = 36
      Height = 13
      Caption = '&Usuario'
      FocusControl = leUsuarios
      Transparent = True
    end
    object Label4: TLabel
      Left = 303
      Top = 49
      Width = 35
      Height = 13
      Caption = 'E&status'
      FocusControl = mruEstatus
      Transparent = True
    end
    object Label5: TLabel
      Left = 8
      Top = 49
      Width = 60
      Height = 13
      Caption = '&Tipo de Caja'
      FocusControl = mruTipoCaja
      Transparent = True
    end
    object laError1: TLabel
      Left = 8
      Top = 288
      Width = 444
      Height = 13
      Caption = 
        'El almacen asignado a esta caja, no tiene activado en su proceso' +
        ' que venda '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object laError2: TLabel
      Left = 8
      Top = 304
      Width = 332
      Height = 13
      Caption = 'El almacen asignado a esta caja, esta fuera de operacion.'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object laError3: TLabel
      Left = 8
      Top = 320
      Width = 287
      Height = 13
      Caption = 'El usuario asignado a esta caja, fue dado de baja.'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 118
      Top = 49
      Width = 35
      Height = 13
      Caption = '&Modulo'
      FocusControl = leModulo
      Transparent = True
    end
    object sbAlmacen: TSpeedButton
      Left = 393
      Top = 24
      Width = 21
      Height = 21
      Hint = 'Actualizar Almacen'
      Glyph.Data = {
        
        04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
      ParentShowHint = False
      ShowHint = True
      OnClick = sbAlmacenClick
    end
    object sbUsuario: TSpeedButton
      Left = 628
      Top = 24
      Width = 21
      Height = 21
      Hint = 'Actualizar Usuarios'
      Glyph.Data = {
        
        04050606060607080B0B0B0B0B0B0B0B0001020202020202030B}
      ParentShowHint = False
      ShowHint = True
      OnClick = sbUsuarioClick
    end
    object leAlmacenes: TDBLookupComboBox
      Left = 269
      Top = 24
      Width = 121
      Height = 21
      DataField = 'clave_almacenes'
      DataSource = dsFuente
      TabOrder = 2
    end
    object deAlmacen: TDBEdit
      Left = 269
      Top = 24
      Width = 121
      Height = 21
      DataField = 'clave_almacen'
      DataSource = dsFuente
      TabOrder = 1
    end
    object deCaja: TDBEdit
      Left = 8
      Top = 24
      Width = 257
      Height = 21
      DataField = 'NOMBRE'
      DataSource = dsFuente
      TabOrder = 0
    end
    object leUsuarios: TDBLookupComboBox
      Left = 419
      Top = 24
      Width = 206
      Height = 21
      DataField = 'nombre_usuarios'
      DataSource = dsFuente
      TabOrder = 4
    end
    object mruEstatus: TDBComboBox
      Left = 303
      Top = 65
      Width = 121
      Height = 21
      DataField = 'ESTATUS'
      DataSource = dsFuente
      Items.Strings = (
        'En Operacion'
        'Fuera de Operacion')
      TabOrder = 7
    end
    object mruTipoCaja: TDBComboBox
      Left = 8
      Top = 65
      Width = 105
      Height = 21
      DataField = 'TIPO_DE_CAJA'
      DataSource = dsFuente
      Items.Strings = (
        'Pagos'
        'Ventas'
        'Desconocido')
      TabOrder = 5
    end
    object deUsuario: TDBEdit
      Left = 419
      Top = 24
      Width = 206
      Height = 21
      DataField = 'nombre_usuario'
      DataSource = dsFuente
      TabOrder = 3
    end
    object leModulo: TDBLookupComboBox
      Left = 118
      Top = 65
      Width = 179
      Height = 21
      DataField = 'descripcion_funcion'
      DataSource = dsFuente
      TabOrder = 6
    end
  end
  inherited fpPadre: TPanel
    Width = 653
    ExplicitWidth = 653
    inherited Panel1: TPanel
      Width = 653
      ExplicitWidth = 653
    end
  end
  inherited Panel2: TPanel
    Top = 446
    Width = 653
    ExplicitTop = 446
    ExplicitWidth = 653
    inherited buInicio: TBitBtn
      Left = 95
      Top = 6
      ExplicitLeft = 95
      ExplicitTop = 6
    end
  end
  inherited dsFuente: TDataSource
    Left = 48
    Top = 320
  end
  inherited qyBotones: TIBQuery
    Left = 77
    Top = 288
  end
  inherited TCapturar: TIBTable
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    IndexFieldNames = 'ID'
    MasterFields = 'R_ID'
    MasterSource = frmExplorar_cajas.dsFuente
    TableName = 'CAJAS'
    Left = 16
    Top = 320
    object TCapturarID: TIntegerField [0]
      FieldName = 'ID'
      Required = True
    end
    object TCapturarNOMBRE: TIBStringField [1]
      FieldName = 'NOMBRE'
      Size = 100
    end
    object TCapturarALMACEN: TIntegerField [2]
      FieldName = 'ALMACEN'
      Required = True
    end
    object TCapturarUSUARIO: TIntegerField [3]
      FieldName = 'USUARIO'
      Required = True
    end
    object TCapturarTIPO_DE_CAJA: TIBStringField [4]
      FieldName = 'TIPO_DE_CAJA'
      Size = 11
    end
    object TCapturarESTATUS: TIBStringField [5]
      FieldName = 'ESTATUS'
    end
    object TCapturarMODULO_FUNCION: TIntegerField [6]
      FieldName = 'MODULO_FUNCION'
      Required = True
    end
    object TCapturarnombre_usuarios: TIBStringField [7]
      FieldKind = fkLookup
      FieldName = 'nombre_usuarios'
      LookupDataSet = qyUsuarios
      LookupKeyFields = 'R_ID'
      LookupResultField = 'R_NOMBRE'
      KeyFields = 'USUARIO'
      Size = 100
      Lookup = True
    end
    object TCapturarclave_almacen: TStringField
      FieldKind = fkCalculated
      FieldName = 'clave_almacen'
      Calculated = True
    end
    object TCapturarclave_almacenes: TStringField
      FieldKind = fkLookup
      FieldName = 'clave_almacenes'
      LookupDataSet = qyAlmacenes
      LookupKeyFields = 'ID'
      LookupResultField = 'CLAVE'
      KeyFields = 'ALMACEN'
      Lookup = True
    end
    object TCapturarnombre_usuario: TIBStringField
      FieldKind = fkCalculated
      FieldName = 'nombre_usuario'
      Size = 100
      Calculated = True
    end
    object TCapturardescripcion_funcion: TStringField
      FieldKind = fkLookup
      FieldName = 'descripcion_funcion'
      LookupDataSet = qyVerModuloFuncion
      LookupKeyFields = 'R_ID'
      LookupResultField = 'R_DESCRIPCION'
      KeyFields = 'MODULO_FUNCION'
      Size = 100
      Lookup = True
    end
  end
  inherited spID: TIBStoredProc
    Left = 80
    Top = 320
    ParamData = <
      item
        DataType = ftInteger
        Name = 'R_ID'
        ParamType = ptOutput
      end
      item
        DataType = ftString
        Name = 'P_QUIEN'
        ParamType = ptInput
        Value = 'CAJAS'
      end>
  end
  object dsAlmacenes: TDataSource
    DataSet = qyAlmacenes
    Left = 296
    Top = 191
  end
  object dsUsuarios: TDataSource
    DataSet = qyUsuarios
    Left = 504
    Top = 71
  end
  object qyUsuarios: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM USUARIOS'
      'WHERE R_BAJA = '#39'No'#39)
    Left = 472
    Top = 71
    object qyUsuariosR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyUsuariosR_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyUsuariosR_BAJA: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_BAJA'
      ReadOnly = True
      Size = 2
    end
  end
  object qyAlmacenes: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM ALMACENES'
      'WHERE (VENDE = '#39'Si'#39') AND (ESTATUS = '#39'En Operacion'#39')')
    Left = 264
    Top = 191
    object qyAlmacenesID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object qyAlmacenesCLAVE: TIBStringField
      FieldName = 'CLAVE'
    end
    object qyAlmacenesNOMBRE: TIBStringField
      FieldName = 'NOMBRE'
      Size = 100
    end
    object qyAlmacenesCOMPRA: TIBStringField
      FieldName = 'COMPRA'
      Size = 2
    end
    object qyAlmacenesVENDE: TIBStringField
      FieldName = 'VENDE'
      Size = 2
    end
    object qyAlmacenesTRANSFORMA: TIBStringField
      FieldName = 'TRANSFORMA'
      Size = 2
    end
    object qyAlmacenesFECHA_ALTA: TDateTimeField
      FieldName = 'FECHA_ALTA'
    end
    object qyAlmacenesRESPONSABLE: TIntegerField
      FieldName = 'RESPONSABLE'
      Required = True
    end
    object qyAlmacenesOBSERVACIONES: TBlobField
      FieldName = 'OBSERVACIONES'
      Size = 8
    end
    object qyAlmacenesSERIE_FACTURA: TIBStringField
      FieldName = 'SERIE_FACTURA'
      Size = 10
    end
    object qyAlmacenesCONSECUTIVO: TIntegerField
      FieldName = 'CONSECUTIVO'
    end
    object qyAlmacenesFECHA_ULT_INV: TDateTimeField
      FieldName = 'FECHA_ULT_INV'
    end
    object qyAlmacenesESTATUS: TIBStringField
      FieldName = 'ESTATUS'
    end
  end
  object qyVerModuloFuncion: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM VER_MODULOS_FUNCION')
    Left = 192
    Top = 111
    object qyVerModuloFuncionR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyVerModuloFuncionR_MODULO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_MODULO'
      ReadOnly = True
    end
    object qyVerModuloFuncionR_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyVerModuloFuncionR_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_DESCRIPCION'
      ReadOnly = True
      Size = 100
    end
    object qyVerModuloFuncionR_INICIALIZACION_DIARIA: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_INICIALIZACION_DIARIA'
      ReadOnly = True
      Size = 2
    end
  end
  object dsVerModuloFuncion: TDataSource
    DataSet = qyVerModuloFuncion
    Left = 224
    Top = 111
  end
end
