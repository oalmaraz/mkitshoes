...
uses
  ...
  IBX.IBCustomDataSet, IBX.IBQuery,   ExtCtrls,
     StdCtrls,
    DBCtrls, ComCtrls, Buttons, Grids, DBGrids;

type
  TSeguimiento = (seSemanal, seQuincenal, seMensual);

  TfrmProyeccion_Cobranza = class(TForm)
    paTitulo: TPanel;
    paBotones: TPanel;
    qyMensual: TIBQuery;
    ...
    dgQuincenal: TDBGrid;
    qySemanalR_EXTENSION_CLAVE: TIBStringField;
    ...
    qyQuincenalR_EXTENSION_CLAVE: TIBStringField;
    ...
    qyMensualR_EXTENSION_CLAVE: TIBStringField;
...
var
  frmProyeccion_Cobranza: TfrmProyeccion_Cobranza;

implementation

uses
  FileCtrl, IBModulo, UfrmEstadoDeCuentas, reglas_de_negocios, IBData,
  UfrmSeleccionarCliente;

{$R *.DFM}



procedure TfrmProyeccion_Cobranza.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
end;

procedure TfrmProyeccion_Cobranza.dgMensualBTN_CXCButtonClick(
...
procedure TfrmProyeccion_Cobranza.buVerClick(Sender: TObject);
begin
   case awrSeguimiento of
...
procedure TfrmProyeccion_Cobranza.FlatButton1Click(Sender: TObject);
var
   tmp : string;
   fileName : string;
begin
...
procedure TfrmProyeccion_Cobranza.FlatButton2Click(Sender: TObject);
var
   tmp : string;
   fileName : string;
begin
...
procedure TfrmProyeccion_Cobranza.FlatButton3Click(Sender: TObject);
var
   tmp : string;
   fileName : string;
begin
...
procedure TfrmProyeccion_Cobranza.FormCreate(Sender: TObject);
begin
...
procedure TfrmProyeccion_Cobranza.edClienteButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure TfrmProyeccion_Cobranza.edClienteKeyPress(Sender: TObject;
  var Key: Char);
begin
{
  if (Key = #13) then
...
procedure TfrmProyeccion_Cobranza.buSemanalClick(
  Sender: TObject);
begin
   awrSeguimiento      := seSemanal;
...
procedure TfrmProyeccion_Cobranza.buMensualClick(
  Sender: TObject);
begin
...
procedure TfrmProyeccion_Cobranza.buQuincenalClick(Sender: TObject);
begin
