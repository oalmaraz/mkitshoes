object frmPresupuesto: TfrmPresupuesto
  Left = 300
  Top = 216
  ActiveControl = dgPartidas
  Caption = 'Presupuestos'
  ClientHeight = 368
  ClientWidth = 682
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnActivate = ibSalirClick
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object dgPartidas: TDBGrid
    Left = 0
    Top = 29
    Width = 682
    Height = 339
    Align = alClient
    DataSource = dsPresupuesto
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
  end
  object paSeleccion: TPanel
    Left = 0
    Top = 0
    Width = 682
    Height = 29
    Align = alTop
    BevelOuter = bvNone
    Color = 15788249
    TabOrder = 1
    ExplicitWidth = 690
    object Label1: TLabel
      Left = 248
      Top = 8
      Width = 60
      Height = 13
      Caption = 'Documentos'
    end
    object Label2: TLabel
      Left = 352
      Top = 8
      Width = 47
      Height = 13
      Caption = 'Uno cada'
    end
    object Label3: TLabel
      Left = 479
      Top = 8
      Width = 12
      Height = 13
      Caption = 'de'
    end
    object edCC: TEdit
      Left = 4
      Top = 4
      Width = 234
      Height = 21
      TabStop = False
      Color = 15788249
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
      Text = '<FALTA>'
    end
    object edDoc: TEdit
      Left = 315
      Top = 4
      Width = 28
      Height = 21
      TabStop = False
      Color = 15788249
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
      Text = '0'
    end
    object edTipo: TEdit
      Left = 406
      Top = 4
      Width = 70
      Height = 21
      TabStop = False
      Color = 15788249
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
      Text = 'Mes'
    end
    object ibAceptar: TBitBtn
      Left = 582
      Top = 2
      Width = 50
      Height = 25
      Hint = 'Corte'
      Caption = '&Aceptar'
      Default = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ModalResult = 6
      NumGlyphs = 2
      ParentFont = False
      ParentShowHint = False
      ShowHint = False
      TabOrder = 3
    end
    object edMonto: TEdit
      Left = 499
      Top = 4
      Width = 78
      Height = 21
      TabStop = False
      Color = 15788249
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 4
      Text = '0'
    end
  end
  object ibSalir: TBitBtn
    Left = 649
    Top = 3
    Width = 38
    Height = 16
    Hint = 'Salir'
    Caption = '&Salir'
    Glyph.Data = {
      
      0000}
    Layout = blGlyphRight
    ModalResult = 7
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
    OnClick = ibSalirClick
  end
  object qyPresupuesto: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM PRESUPUESTO(:P_MONTO)')
    Left = 232
    Top = 296
    ParamData = <
      item
        DataType = ftString
        Name = 'P_MONTO'
        ParamType = ptUnknown
        Value = '0.00'
      end>
    object qyPresupuestoR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyPresupuestoR_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLAVE'
      ReadOnly = True
    end
    object qyPresupuestoR_NOMBRE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_NOMBRE'
      ReadOnly = True
      Size = 100
    end
    object qyPresupuestoR_DOC_MES: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_DOC_MES'
      ReadOnly = True
    end
    object qyPresupuestoR_MENSUAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_MENSUAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyPresupuestoR_DOC_QUINCENAL: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_DOC_QUINCENAL'
      ReadOnly = True
    end
    object qyPresupuestoR_QUINCENAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_QUINCENAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyPresupuestoR_DOC_SEMANAL: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_DOC_SEMANAL'
      ReadOnly = True
    end
    object qyPresupuestoR_SEMANAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_SEMANAL'
      ReadOnly = True
      DisplayFormat = '$ ###,###,##0.00'
    end
    object qyPresupuestoR_FINANCIAMIENTO: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_FINANCIAMIENTO'
      ReadOnly = True
    end
    object qyPresupuestoR_FINANCIAMIENTO_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_FINANCIAMIENTO_CLAVE'
      ReadOnly = True
    end
    object qyPresupuestoR_FINANCIAMIENTO_DESCRIPCION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_FINANCIAMIENTO_DESCRIPCION'
      ReadOnly = True
      Size = 100
    end
    object qyPresupuestoR_FINANCIAMIENTO_PORCENTAJE: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_FINANCIAMIENTO_PORCENTAJE'
      ReadOnly = True
    end
    object qyPresupuestoR_ROWS: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ROWS'
      ReadOnly = True
    end
    object qyPresupuestoR_COLOR: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_COLOR'
      ReadOnly = True
      Size = 6
    end
  end
  object dsPresupuesto: TDataSource
    DataSet = qyPresupuesto
    Left = 264
    Top = 296
  end
end
