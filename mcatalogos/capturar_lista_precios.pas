...
uses
  ...
  Vcl.ComCtrls, dxCore, cxDateUtils, cxDBEdit, cxDropDownEdit, cxCalendar;

type
  TfrmCapturar_lista_precios = class(TfrmCapturarMDI)
    Label6: TLabel;
    edMaterial: TcxButtonEdit;
    deFecha: TcxDBDateEdit;
    Label9: TLabel;
    cePrecio_Minimo: TcxDBCurrencyEdit;
    Label14: TLabel;
    cePrecio_Maximo: TcxDBCurrencyEdit;
    Label3: TLabel;
    Label22: TLabel;
    edDescripcion: TEdit;
...
begin
  ...
  deFecha.Date := dsFuente.DataSet.FieldByName('FECHA').AsDateTime;
end;
...
begin
   ...
   begin
      if deFecha.Focused then
         cePrecio_Minimo.SetFocus;
      entero   := Int(deFecha.Date);
      fraccion := Frac(deFecha.Date);
      if entero > 0 then
      begin
         if fraccion = 0 then
         begin
            dsFuente.DataSet.FieldByName('FECHA').AsDateTime := deFecha.Date  ;//StrToDateTime ( deFecha.text) + time;
         end;
      end;
      inherited;
   end;
