...
uses
  ...
  IBX.IBCustomDataSet, IBX.IBQuery,   ExtCtrls,
     StdCtrls,
    Grids, DBGrids, ComCtrls, DBCtrls,
     Buttons;

type
  TSeguimiento = (seSemanal, seQuincenal, seMensual);

  TfrmProyeccion_Pagos = class(TForm)
    paTitulo: TPanel;
    paBotones: TPanel;
    qyMensual: TIBQuery;
    ...
    qyMensualR_VENCIDO: TFloatField;
    qyMensualR_DIA_CERO: TFloatField;
    qyMensualR_1_A_30: TFloatField;
    qyMensualR_TOTAL_CON_DIA_CERO: TFloatField;
    qyMensualR_TOTAL_SIN_DIA_CERO: TFloatField;
    qySemanalR_VENCIDO: TFloatField;
    qySemanalR_DIA_CERO: TFloatField;
    qySemanalR_1_A_6: TFloatField;
    qySemanalR_TOTAL_CON_DIA_CERO: TFloatField;
    qySemanalR_TOTAL_SIN_DIA_CERO: TFloatField;
    qyQuincenalR_VENCIDO: TFloatField;
    qyQuincenalR_DIA_CERO: TFloatField;
    qyQuincenalR_1_A_15: TFloatField;
    qyQuincenalR_TOTAL_CON_DIA_CERO: TFloatField;
    qyQuincenalR_TOTAL_SIN_DIA_CERO: TFloatField;
    buImprimirS: TBitBtn;
...
var
  frmProyeccion_Pagos: TfrmProyeccion_Pagos;

implementation

uses
  FileCtrl, IBModulo, UfrmEstadoDeCuentas, reglas_de_negocios, IBData,
  UfrmSeleccionarProveedor, UfrmCuentasPorPagar, URPT_PROYECCION_SEM_DET_PAGOS;

{$R *.DFM}

procedure TfrmProyeccion_Pagos.cambiarFocus;
begin
...
procedure TfrmProyeccion_Pagos.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
   Action := caFree;
end;

procedure TfrmProyeccion_Pagos.dgMensualBTN_CXCButtonClick(
...
procedure TfrmProyeccion_Pagos.buVerClick(Sender: TObject);
begin
   case awrSeguimiento of
...
procedure TfrmProyeccion_Pagos.FlatButton1Click(Sender: TObject);
var
   tmp : string;
   fileName : string;
begin
...
procedure TfrmProyeccion_Pagos.FlatButton2Click(Sender: TObject);
var
   tmp : string;
   fileName : string;
begin
...
procedure TfrmProyeccion_Pagos.FlatButton3Click(Sender: TObject);
var
   tmp : string;
   fileName : string;
begin
...
procedure TfrmProyeccion_Pagos.FormCreate(Sender: TObject);
begin
...
procedure TfrmProyeccion_Pagos.edProveedorButtonClick(Sender: TObject;
  AbsoluteIndex: Integer);
begin
...
procedure TfrmProyeccion_Pagos.edProveedorKeyPress(Sender: TObject;
  var Key: Char);
begin
{
  if (Key = #13) then
...
procedure TfrmProyeccion_Pagos.buSemanalClick(
  Sender: TObject);
begin
   awrSeguimiento      := seSemanal;
...
procedure TfrmProyeccion_Pagos.buMensualClick(
  Sender: TObject);
begin
...
procedure TfrmProyeccion_Pagos.buQuincenalClick(Sender: TObject);
begin
...
procedure TfrmProyeccion_Pagos.buImprimirSClick(Sender: TObject);
begin
   cambiarFocus;
   RPT_PROYECCION_SEM_DET_PAGOS := TRPT_PROYECCION_SEM_DET_PAGOS.Create(Self);
   try
      RPT_PROYECCION_SEM_DET_PAGOS.FECHA     := edFecha.Date;
      RPT_PROYECCION_SEM_DET_PAGOS.PROVEEDOR := edProveedor.Tag;
      RPT_PROYECCION_SEM_DET_PAGOS.open;
      RPT_PROYECCION_SEM_DET_PAGOS.imprimir;
      RPT_PROYECCION_SEM_DET_PAGOS.openResumen;
      RPT_PROYECCION_SEM_DET_PAGOS.imprimirResumen;
   finally
      RPT_PROYECCION_SEM_DET_PAGOS.Free;
   end;
end;
