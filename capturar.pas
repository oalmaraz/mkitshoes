...
uses
  USqlAnaliza,  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, IBX.IBQuery,
  hijo, IBX.IBStoredProc, IBX.IBTable,  Db, IBX.IBCustomDataSet,
   StdCtrls, Mask, DBCtrls,  jpeg,  ExtCtrls,
   ComCtrls, Buttons , dbgrids, cxGrid, cxGridDBTableView;
...
type
  ...
  public
    { Public declarations }
    procedure SetRejilla(varRejilla : TcxGridDBTableView);
  published
     property Grid : TcxGridDBTableView write SetRejilla;
  end;
...
procedure TfrmCapturar.SetRejilla(varRejilla : TcxGridDBTableView);
begin
   rejilla := varRejilla;
end;
...
begin
  ...
  begin
    ...
    begin
       if awrSQL <> nil then
       begin
          if rejilla.DataController.DataSource.DataSet.RecordCount = 1 then
          begin
             rejilla.DataController.DataSource.DataSet.Close;
             rejilla.DataController.DataSource.DataSet.Open;
          end;
       end;
    end
    else
...
begin
  ...
  begin
     ...
     rejilla.DataController.DataSource.DataSet.First;
  end;
end;
...
begin
  ...
  begin
     if dsFuente.DataSet.State in [dsEdit, dsInsert] then
        dsFuente.DataSet.Cancel;
     rejilla.DataController.DataSource.DataSet.Prior;
  end;
end;
...
begin
  ...
  begin
     if dsFuente.DataSet.State in [dsEdit, dsInsert] then
        dsFuente.DataSet.Cancel;
     rejilla.DataController.DataSource.DataSet.Next;
...
begin
  ...
  begin
     if dsFuente.DataSet.State in [dsEdit, dsInsert] then
        dsFuente.DataSet.Cancel;
     rejilla.DataController.DataSource.DataSet.Last;
  end;
end;
