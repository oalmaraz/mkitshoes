unit explorar_anticipos;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  explorarMDI, Db,  Grids, DBGrids, Buttons,   
  Menus, IBX.IBCustomDataSet, IBX.IBQuery, ComCtrls, StdCtrls, Mask, DBCtrls,
...
type
  TfrmExplorar_anticipos = class(TfrmExplorarMDI)
...
var
  frmExplorar_anticipos: TfrmExplorar_anticipos;
...
procedure TfrmExplorar_anticipos.Mostrar(p_cuantos : integer);
begin
   if p_cuantos = 1 then
   begin
      OnClickMio(Self);
...
procedure TfrmExplorar_anticipos.OnClickMio(Sender : TObject);
begin
...
procedure TfrmExplorar_anticipos.armar_sql;
{var
   primero : boolean;
   valor   : string;}
begin
...
constructor TfrmExplorar_anticipos.CrearParaPagos(AOwner : TComponent; PX, PY : TPoint; pConstructor : integer);
begin
   Create(AOwner);
...
procedure TfrmExplorar_anticipos.FormCreate(Sender: TObject);
begin
  inherited;
  EXP_NVO     := true;
  CAMPO_FECHA := 'R_FECHA';
  FForma      := 'frmCapturar_anticipos';
end;
