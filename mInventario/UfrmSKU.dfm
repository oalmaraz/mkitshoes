object frmSKU: TfrmSKU
  Left = 638
  Top = 202
  Caption = 'Busqueda SKU'
  ClientHeight = 286
  ClientWidth = 499
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object paTitulo: TPanel
    Left = 0
    Top = 0
    Width = 499
    Height = 20
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Busqueda SKU'
    Color = 12615680
    Font.Charset = ANSI_CHARSET
    Font.Color = clWhite
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    ExplicitWidth = 507
  end
  object paBotones: TPanel
    Left = 0
    Top = 246
    Width = 499
    Height = 40
    Align = alBottom
    TabOrder = 3
    ExplicitTop = 251
    ExplicitWidth = 507
    object buSalir: TBitBtn
      Left = 420
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Regresar'
      Glyph.Data = {
        
        A7B7A7A7B7A7A7B7A6A6B6A6A6B8DBDBE1FFFFFFFFFFFFFFFFFF}
      ModalResult = 5
      TabOrder = 0
    end
    object buGuardar: TBitBtn
      Left = 342
      Top = 5
      Width = 75
      Height = 30
      Caption = '&Guardar'
      Glyph.Data = {
        
        0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000FF0000}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = buGuardarClick
    end
  end
  object paMaterial: TPanel
    Left = 0
    Top = 86
    Width = 499
    Height = 162
    Align = alTop
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 1
    ExplicitWidth = 507
    object Label2: TLabel
      Left = 8
      Top = -1
      Width = 31
      Height = 16
      Caption = 'Clave'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object laClave: TDBText
      Left = 8
      Top = 16
      Width = 137
      Height = 21
      Alignment = taCenter
      Color = clBackground
      DataField = 'R_MATERIAL_C'
      DataSource = dsSKU
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label1: TLabel
      Left = 152
      Top = -1
      Width = 65
      Height = 16
      Caption = 'Descripcion'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object laDescripcion: TDBText
      Left = 152
      Top = 16
      Width = 337
      Height = 21
      Color = clBackground
      DataField = 'R_MATERIAL_D'
      DataSource = dsSKU
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label3: TLabel
      Left = 152
      Top = 39
      Width = 80
      Height = 16
      Caption = 'Codigo Barras'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object laCBarras: TDBText
      Left = 152
      Top = 56
      Width = 200
      Height = 21
      Color = clBackground
      DataField = 'R_MATERIAL_CB'
      DataSource = dsSKU
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label4: TLabel
      Left = 152
      Top = 79
      Width = 41
      Height = 16
      Caption = 'Medida'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object laMedida: TDBText
      Left = 152
      Top = 96
      Width = 200
      Height = 21
      Color = clBackground
      DataField = 'R_MEDIDA_D'
      DataSource = dsSKU
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label5: TLabel
      Left = 152
      Top = 119
      Width = 30
      Height = 16
      Caption = 'Color'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object laColor: TDBText
      Left = 152
      Top = 136
      Width = 200
      Height = 21
      Color = clBackground
      DataField = 'R_COLOR_N'
      DataSource = dsSKU
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object Label6: TLabel
      Left = 8
      Top = 39
      Width = 50
      Height = 16
      Caption = 'Cantidad'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object laCantidad: TDBText
      Left = 8
      Top = 56
      Width = 137
      Height = 21
      Alignment = taCenter
      Color = clBackground
      DataField = 'R_CANTIDAD'
      DataSource = dsSKU
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
  end
  object paCantidad: TPanel
    Left = 0
    Top = 20
    Width = 499
    Height = 66
    Align = alTop
    BevelOuter = bvNone
    Color = clWhite
    TabOrder = 2
    ExplicitWidth = 507
    object Label7: TLabel
      Left = 232
      Top = 12
      Width = 35
      Height = 19
      Caption = '&Serie'
      FocusControl = edSerie
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object edSerie: TEdit
      Left = 8
      Top = 35
      Width = 482
      Height = 27
      CharCase = ecUpperCase
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      MaxLength = 100
      ParentFont = False
      TabOrder = 0
      OnKeyPress = edSerieKeyPress
    end
  end
  object spSKU_CONFIRMACION_COMPLETA: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'SKU_CONFIRMACION_COMPLETA'
    Left = 112
    Top = 208
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_MI'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_MATERIAL'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_MEDIDA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'P_COLOR'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'P_SERIE'
        ParamType = ptInput
      end>
  end
  object qySKU: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM T_SKU(:P_ALMACEN, :P_SKU)')
    Left = 48
    Top = 208
    ParamData = <
      item
        DataType = ftString
        Name = 'P_ALMACEN'
        ParamType = ptUnknown
        Value = '0'
      end
      item
        DataType = ftString
        Name = 'P_SKU'
        ParamType = ptUnknown
        Value = 'ABC'
      end>
    object qySKUR_ID: TIntegerField
      FieldName = 'R_ID'
      Origin = 'T_SKU.R_ID'
    end
    object qySKUR_MATERIAL: TIntegerField
      FieldName = 'R_MATERIAL'
      Origin = 'T_SKU.R_MATERIAL'
    end
    object qySKUR_MATERIAL_C: TIBStringField
      FieldName = 'R_MATERIAL_C'
      Origin = 'T_SKU.R_MATERIAL_C'
    end
    object qySKUR_MATERIAL_D: TIBStringField
      FieldName = 'R_MATERIAL_D'
      Origin = 'T_SKU.R_MATERIAL_D'
      Size = 100
    end
    object qySKUR_MATERIAL_CB: TIBStringField
      FieldName = 'R_MATERIAL_CB'
      Origin = 'T_SKU.R_MATERIAL_CB'
    end
    object qySKUR_MEDIDA: TIntegerField
      FieldName = 'R_MEDIDA'
      Origin = 'T_SKU.R_MEDIDA'
    end
    object qySKUR_MEDIDA_D: TIBStringField
      FieldName = 'R_MEDIDA_D'
      Origin = 'T_SKU.R_MEDIDA_D'
    end
    object qySKUR_COLOR: TIntegerField
      FieldName = 'R_COLOR'
      Origin = 'T_SKU.R_COLOR'
    end
    object qySKUR_COLOR_N: TIBStringField
      FieldName = 'R_COLOR_N'
      Origin = 'T_SKU.R_COLOR_N'
      Size = 50
    end
    object qySKUR_CANTIDAD: TFloatField
      FieldName = 'R_CANTIDAD'
      Origin = 'T_SKU.R_CANTIDAD'
      DisplayFormat = '###,###,##0'
    end
  end
  object dsSKU: TDataSource
    DataSet = qySKU
    Left = 16
    Top = 208
  end
end
