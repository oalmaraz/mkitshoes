inherited frmVer_notas_de_credito_en_nc: TfrmVer_notas_de_credito_en_nc
  Left = 249
  Top = 342
  Caption = 'Notas de Credito en Nota de Credito'
  ClientHeight = 329
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited fpPadre: TPanel
    Height = 58
    ExplicitHeight = 58
    inherited Panel1: TPanel
      TabOrder = 2
    end
    object ibEliminar: TBitBtn
      Left = 2
      Top = 20
      Width = 37
      Height = 34
      Hint = 'Eliminar Cotizacion(es) en Pedidos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        FFFF60CF6060CF60404740707770404740FFCFCF60303060CF60}
      Layout = blGlyphRight
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = ibEliminarClick
    end
    object ibSalir: TBitBtn
      Left = 41
      Top = 20
      Width = 37
      Height = 34
      Hint = 'Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphRight
      ModalResult = 1
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
    end
  end
  object dgPartidas: TDBGrid
    Left = 0
    Top = 58
    Width = 568
    Height = 271
    Align = alClient
    DataSource = dsNC
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
  end
  object qyNC: TIBQuery
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT *'
      'FROM NOTAS_DE_CREDITO_EN_NC(:P_NC)')
    Left = 40
    Top = 112
    ParamData = <
      item
        DataType = ftString
        Name = 'P_NC'
        ParamType = ptUnknown
        Value = '0'
      end>
    object qyNCR_ID: TIntegerField
      FieldKind = fkInternalCalc
      FieldName = 'R_ID'
      ReadOnly = True
    end
    object qyNCR_CLAVE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLAVE'
      ReadOnly = True
    end
    object qyNCR_FECHA_ALTA: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_ALTA'
      ReadOnly = True
    end
    object qyNCR_TOTAL: TFloatField
      FieldKind = fkInternalCalc
      FieldName = 'R_TOTAL'
      ReadOnly = True
    end
    object qyNCR_CLIENTE: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_CLIENTE'
      ReadOnly = True
      Size = 100
    end
    object qyNCR_EXTENSION: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_EXTENSION'
      ReadOnly = True
      Size = 100
    end
    object qyNCR_REFERENCIA: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'R_REFERENCIA'
      ReadOnly = True
    end
    object qyNCR_FECHA_COMPRA: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_COMPRA'
      ReadOnly = True
    end
    object qyNCR_FECHA_VENCIMIENTO: TDateTimeField
      FieldKind = fkInternalCalc
      FieldName = 'R_FECHA_VENCIMIENTO'
      ReadOnly = True
    end
  end
  object dsNC: TDataSource
    DataSet = qyNC
    Left = 72
    Top = 112
  end
  object spACT_COT_EN_PEDIDO: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'ACT_COT_EN_PEDIDO'
    Left = 72
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_COTIZACION'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_PEDIDO'
        ParamType = ptInput
        Value = '0'
      end>
  end
  object spDEL_NC_A_NC: TIBStoredProc
    Database = dmIBModulo.ibSistema
    Transaction = dmIBModulo.ibSistemaTransaccion
    StoredProcName = 'DEL_NC_A_NC'
    Left = 40
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'P_NC_ORIGEN'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'P_NC_DESTINO'
        ParamType = ptInput
        Value = '0'
      end>
  end
  object dsPartidas: TDataSource
    Left = 72
    Top = 176
  end
  object dsEncabezado: TDataSource
    OnDataChange = dsEncabezadoDataChange
    Left = 40
    Top = 176
  end
end
